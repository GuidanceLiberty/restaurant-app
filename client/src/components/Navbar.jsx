  ...
  return (
    <>
      <nav className="navbar">
        <div
          className="nav-menu sm:block md:hidden lg:hidden flex items-center text-orange-400 cursor-pointer"
          onClick={() => setShowMenu(!showMenu)}
        >
          <img
            src={logo}
            alt=""
            className="w-7 h-7 sm:w-7 sm:h-7 md:w-5 md:h-5 lg:w-4 lg:h-4"
          />
        </div>
          ...
          <div className="!flex-col !items-center py-3">
            {categories?.data &&
              categories?.data?.map((category) => (
                <NavLink
                  to={`/category-page/${category?._id}`}
                  key={category?._id}
                  className={({ isActive }) => {
                    return isActive
                      ? "side-menu-active mb-1"
                      : "side-menu mb-1";
                  }}
                >
                  {/* <span><RiCake2Line className='text-orange-400' /></span> */}
                  <img
                    src={category?.photo || placeholder}
                    alt={category?.name}
                    className="text-orange-400 w-4 h-4 rounded-full"
                  />
      ...
      {showUserMenu && (
        <div className="bg-white side-menus h-fit w-fit border border-accent/40 p-4 rounded-md shadow-md !mt-2 z-30 absolute !right-8">
          <div className="!flex-col !items-center py-3">
            ...
            >
              <span>
                <RiUser2Line className="text-orange-400 h-4 w-4" />
              </span>
              <span>{user?.name}</span>
            </NavLink>
            ...
            {user?.role === "admin" && (
              <>
                ...
                >
                  <span>
                    <RiGridLine className="text-orange-400 h-4 w-4" />
                  </span>
                ...
                >
                  <span>
                    <RiBuilding4Line className="text-orange-400 h-4 w-4" />
                  </span>
                ...
                >
                  <span>
                    <RiTeamLine className="text-orange-400 h-4 w-4" />
                  </span>
            ...
            <div onClick={handleLogout} className="side-menu cursor-pointer">
              <span>
                <RiLogoutCircleRLine className="text-red-600 h-4 w-4" />
              </span>
